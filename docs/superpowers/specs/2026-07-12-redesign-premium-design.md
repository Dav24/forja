...
- **Contenido de animaciones de ejercicio:** cientos de ejercicios × animación = la decisión de mayor costo del rediseño (licenciar pack/API vs producir Lottie propios). Además exige que `generate-plan` genere ejercicios mapeables a un catálogo (hoy la IA escribe nombres libres). Se dimensiona en la spec de implementación RN.
